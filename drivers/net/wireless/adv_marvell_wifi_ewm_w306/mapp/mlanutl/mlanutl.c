...
static int process_wnmpscfg(int argc, char *argv[]);
...
/**
 *  @brief Process Power Save Features configuration
 *  @param argc   Number of arguments
 *  @param argv   A pointer to arguments array
 *  @return     MLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int
process_wnmpscfg(int argc, char *argv[])
{
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	struct ifreq ifr;
	int li = 0, arg_num = 0, ret = 0, cmd_found = 0;
	char *args[30], *pos = NULL;
	wnm_ps_bss_max_idle_tlv_t *ptlv_bss_max_idle = NULL;
	wnm_ps_dms_tlv_t *ptlv_dms = NULL;
	tclas_element_tlv_t *ptlv_tclas = NULL;
	t_u16 sub_tlv_len = 0, sub_tlv_num = 0;
	int i = 0;
	t_u16 total_tlv_len = 0;
	t_u16 tlv_processed = 0;
	...
	if (argc != 4) {
		ret = -EINVAL;
		printf("Insufficient parameters\n");
		printf("For STA interface: mlanutl mlanX wnmpscfg <config file>\n");
		goto done;
	}
	...
	prepare_buffer(buffer, argv[2], 0, NULL);
	cmd_len = strlen(CMD_MARVELL) + strlen(argv[2]);
	total_tlv_len += cmd_len;
	/* length of the total tlv data following */
	total_tlv_len += sizeof(t_u16);

	/* Prepare buffer with TLV per feature, TLV config file */
	fp = fopen(argv[3], "r");
	if (fp == NULL) {
		perror("fopen");
		fprintf(stderr, "Cannot open file %s\n", argv[3]);
	...
	while (config_get_line(line, MAX_CONFIG_LINE, fp, &li, &pos)) {
		...
		if (0) {
		} else if (strcmp(args[0], "BSSMaxIdle") == 0) {
			/* Add new TLV */
			ptlv_bss_max_idle =
				(wnm_ps_bss_max_idle_tlv_t *) (buffer +
							       total_tlv_len);
			ptlv_bss_max_idle->header.type = TLV_TYPE_BSS_MAX_IDLE;
			ptlv_bss_max_idle->header.len =
				sizeof(wnm_ps_bss_max_idle_cfg_t);
			/* complete TLV len */
			total_tlv_len +=
				ptlv_bss_max_idle->header.len +
				sizeof(MrvlIEtypesHeader_t);
		} else if ((ptlv_bss_max_idle != NULL) &&
			   (strcmp(args[0], "BSSMaxIdleAction") == 0)) {
			ptlv_bss_max_idle->bss_max_idle_cfg.
				bss_max_idle_action =
				(t_u16)a2hex_or_atoi(args[1]);
		} else if ((ptlv_bss_max_idle != NULL) &&
			   (strcmp(args[0], "BSSMaxIdleEnable") == 0)) {
			ptlv_bss_max_idle->bss_max_idle_cfg.
				bss_max_idle_enable =
				(t_u8)a2hex_or_atoi(args[1]);
		} else if (strcmp(args[0], "DMS") == 0) {
			/* Add new TLV */
			ptlv_dms =
				(wnm_ps_dms_tlv_t *) (buffer + total_tlv_len);
			ptlv_dms->header.type = TLV_TYPE_DMS;
			total_tlv_len += sizeof(MrvlIEtypesHeader_t);
		} else if ((ptlv_dms != NULL) &&
			   (strcmp(args[0], "DMSAction") == 0)) {
			ptlv_dms->dms_cfg.dms_action =
				(t_u16)a2hex_or_atoi(args[1]);
			total_tlv_len += sizeof(ptlv_dms->dms_cfg.dms_action);
		} else if ((ptlv_dms != NULL) &&
			   (strcmp(args[0], "DMSEnable") == 0)) {
			ptlv_dms->dms_cfg.dms_enable =
				(t_u8)a2hex_or_atoi(args[1]);
			total_tlv_len += sizeof(ptlv_dms->dms_cfg.dms_enable);
		} else if ((ptlv_dms != NULL) &&
			   (strcmp(args[0], "DMSRequestType") == 0)) {
			ptlv_dms->dms_req.dms_req_type =
				(t_u16)a2hex_or_atoi(args[1]);
			total_tlv_len += sizeof(ptlv_dms->dms_req.dms_req_type)
				+ sizeof(ptlv_dms->dms_req.dms_desc_elements)
				+ sizeof(ptlv_dms->dms_req.dms_total_data_len);
		} else if ((ptlv_dms != NULL) &&
			   (strcmp(args[0], "DMStclas_ipv4_1") == 0)) {
			/* Add sub TLV as a parameter for DMS */
			ptlv_tclas =
				(tclas_element_tlv_t *) (ptlv_dms->dms_req.
							 dms_desc_tlv +
							 sub_tlv_len);
			ptlv_tclas->header.type = TLV_TYPE_TCLAS_ELEMENT;
			ptlv_tclas->header.len =
				sizeof(IEEEtypes_TCLAS_Base_t) +
				sizeof(IEEEtypes_TCLAS_IPv4_t);
			/* Complete sub TLV len */
			sub_tlv_len +=
				(sizeof(MrvlIEtypesHeader_t) +
				 ptlv_tclas->header.len);
			sub_tlv_num++;
			...
			total_tlv_len +=
				((sizeof(MrvlIEtypesHeader_t) +
				  ptlv_tclas->header.len));
			/* no of elements in DMS Desc */
			ptlv_dms->dms_req.dms_desc_elements = sub_tlv_num;
			/* total DMS Desc data len */
			ptlv_dms->dms_req.dms_total_data_len = sub_tlv_len;
			/* total DMS TLV len */
			ptlv_dms->header.len = sub_tlv_len;
			/* Fill the TCLAS element */
			ptlv_tclas->tclas_ie.tclas_base.element_id = TCLAS;
			ptlv_tclas->tclas_ie.tclas_base.element_len =
				sizeof(IEEEtypes_TCLAS_Base_t)
				+ sizeof(IEEEtypes_TCLAS_IPv4_t)
				- sizeof(IEEEtypes_ElementId_e)
				- sizeof(t_u8);
			/* frame classifier type 4 */
			ptlv_tclas->tclas_ie.tclas_base.classifier_type =
				TCLAS_CLASSIFIER_TYPE_4;
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4UserPriority_1") == 0)) {
			ptlv_tclas->tclas_ie.tclas_base.user_priority =
				(t_u8)a2hex_or_atoi(args[1]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4ClassifierMask_1") == 0)) {
			ptlv_tclas->tclas_ie.tclas_base.classifier_mask =
				(t_u8)a2hex_or_atoi(args[1]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4Version_1") == 0)) {
			ptlv_tclas->tclas_ie.classifier.ipv4.version =
				(t_u8)a2hex_or_atoi(args[1]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4SourceIpAddress_1") == 0)) {
			for (i = 0; i < (arg_num - 1); i++)
				ptlv_tclas->tclas_ie.classifier.ipv4.
					source_ip_addr[i] =
					(t_u8)a2hex_or_atoi(args[1 + i]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4DestIpAddress_1") == 0)) {
			for (i = 0; i < (arg_num - 1); i++)
				ptlv_tclas->tclas_ie.classifier.ipv4.
					dest_ip_addr[i] =
					(t_u8)a2hex_or_atoi(args[1 + i]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4SourcePort_1") == 0)) {
			for (i = 0; i < (arg_num - 1); i++)
				ptlv_tclas->tclas_ie.classifier.ipv4.
					source_port[i] =
					(t_u8)a2hex_or_atoi(args[1 + i]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4DestPort_1") == 0)) {
			for (i = 0; i < (arg_num - 1); i++)
				ptlv_tclas->tclas_ie.classifier.ipv4.
					dest_port[i] =
					(t_u8)a2hex_or_atoi(args[1 + i]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4DSCP_1") == 0)) {
			ptlv_tclas->tclas_ie.classifier.ipv4.dscp =
				(t_u8)a2hex_or_atoi(args[1]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4Protocol_1") == 0)) {
			ptlv_tclas->tclas_ie.classifier.ipv4.protocol =
				(t_u8)a2hex_or_atoi(args[1]);
		} else if (((ptlv_dms != NULL && ptlv_tclas != NULL)) &&
			   (strcmp(args[0], "DMSipv4Reserved_1") == 0)) {
			ptlv_tclas->tclas_ie.classifier.ipv4.reserved =
				(t_u8)a2hex_or_atoi(args[1]);
		}
	}
	/* Add remaining length to dms TLV */
	if (ptlv_dms) {
		ptlv_dms->header.len +=
			sizeof(wnm_ps_dms_cfg_t) + sizeof(wnm_ps_dms_req_t);
		/* Sanity for no of elements */
		if (ptlv_dms->dms_req.dms_desc_elements > 1) {
			printf("Optional elements not supported\n");
			ret = -EFAULT;
			goto done;
		}
	}

	/* Accomodate the total TLV data length into buffer */
	total_tlv_len -= (cmd_len + sizeof(t_u16));
	memcpy(buffer + cmd_len, &total_tlv_len, sizeof(total_tlv_len));
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: wnmpscfg fail\n");
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}

	/* Process result */
	memcpy(&total_tlv_len, buffer, sizeof(t_u16));
	tlv = (MrvlIEtypesHeader_t *)(buffer + sizeof(t_u16));
	tlv_processed = 0;

	while (total_tlv_len >= sizeof(MrvlIEtypesHeader_t)) {
		if (total_tlv_len < (sizeof(MrvlIEtypesHeader_t) + tlv->len))
			break;

		switch (tlv->type) {
		case TLV_TYPE_BSS_MAX_IDLE:
			ptlv_bss_max_idle = (wnm_ps_bss_max_idle_tlv_t *) tlv;
			printf("BSSMaxIdle: 0x%01x\n",
			       ptlv_bss_max_idle->bss_max_idle_cfg.
			       bss_max_idle_enable);
			tlv_processed +=
		...
		total_tlv_len -= (sizeof(MrvlIEtypesHeader_t) + tlv->len);
