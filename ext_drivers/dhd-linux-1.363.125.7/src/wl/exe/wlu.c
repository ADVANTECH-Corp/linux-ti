 ...
 *
 * $Id: wlu.c 666957 2017-09-04 05:45:12Z $
 */
...
#ifdef TCPKAOE
static cmd_func_t wl_tcpka_conn_add;
static cmd_func_t wl_tcpka_conn_enable;
#endif
...
#ifdef TCPKAOE
	{"tcpka_conn_add", wl_tcpka_conn_add, -1, WLC_SET_VAR,
	"Add Tcp Keep Alive Session\n"
	"\tUsage: tcpka_conn_add <period> <timeout> <dst_mac> <src_ip> <dst_ip> <ip_id> <src_port> <dst_port> <seq> <ack> <data>\n"
	"\t<period>: the period time of send tcp packet for keepalive (keep tcp session), a decimal value, in seconds.\n"
	"\t<timeout>: the period time of re-send tcp packet, if not receive ACK from server, a decimal value, in seconds.\n"
	"\t<dst_mac>: destination mac address\n"
	"\t<src_ip>: source IP (local)"
	"\t<dst_ip>: destination IP (remote)\n"
	"\t<ip_id>: the starting IP frame identification number\n"
	"\t<src_port>: source port\n"
	"\t<dst_port>: destination port\n"
	"\t<seq>: Sequence number of the TCP frame\n"
	"\t<ack>: Acknowledge number of the TCP frame\n"
	"\t<data>: The data in payload (hex)"},
	{"tcpka_conn_enable", wl_tcpka_conn_enable, -1, WLC_SET_VAR,
	"Enable TCPKA feature\n"
	"\tUsage: tcpka_conn_enable <ON/OFF> \n"
	"\t<1/0>: Enable/Disable\n"},
#endif
	{ NULL, NULL, 0, 0, NULL }
};
...
#ifdef TCPKAOE
/* Add TCP Keep alive connection info */
static int
wl_tcpka_conn_add(void *wl, cmd_t *cmd, char **argv)
{
	int ret = 0;
	...
	/* Store period */
	tcpka.period = (uint16)htod32(strtoul(*argv, NULL, 0));
	...
	/* Store timeout */
	tcpka.timeout = (uint16)htod32(strtoul(*argv, NULL, 0));

	if (!*++argv)
		return BCME_USAGE_ERROR;

	/* Store Destination MAC */
	if (!wl_ether_atoe(*argv, (struct ether_addr *)&tcpka.dst_mac))
		return BCME_USAGE_ERROR;
	...
	/* Store Src IP */
	if (!wl_atoip(*argv, (struct ipv4_addr *)&tcpka.src_ip))
		return BCME_USAGE_ERROR;
	...
	/* Store Dst IP */
	if (!wl_atoip(*argv, (struct ipv4_addr *)&tcpka.dst_ip))
		return BCME_USAGE_ERROR;
	...
	/* Store IP ID */
	tcpka.ipid = (uint16)htod32(strtoul(*argv, NULL, 0));
	...
	/* Store src port */
	tcpka.srcport = (uint16)htod32(strtoul(*argv, NULL, 0));

	if (!*++argv)
		return BCME_USAGE_ERROR;

	/* Store dst port */
	tcpka.dstport = (uint16)htod32(strtoul(*argv, NULL, 0));

	if (!*++argv)
		return BCME_USAGE_ERROR;

	/* Store sequence number  */
	tcpka.seq = (uint32)htod32(strtoul(*argv, NULL, 0));


	if (!*++argv)
		return BCME_USAGE_ERROR;

	/* Store acknowledge number  */
	tcpka.ack = (uint32)htod32(strtoul(*argv, NULL, 0));


	if (!*++argv)
		return BCME_USAGE_ERROR;

	/* Store data of packet payload  */
	memset(tcpka.data, '0', sizeof(tcpka.data));
	tcpka.data_len = wl_pattern_atoh(*argv, (char *)&tcpka.data[0]);


	if ((ret = wlu_iovar_getbuf(wl, cmd->name, &tcpka, sizeof(tcpka), buf,
					WLC_IOCTL_MAXLEN))) {
		printf("failed:%s %x\n",cmd->name,ret);
		return (ret);
	}
...
static int
wl_tcpka_conn_enable(void *wl, cmd_t *cmd, char **argv)
{
	int ret = 0;
	uint32 flag;
	...
	flag = (uint16)htod32(strtoul(*argv, NULL, 0));


	/* This passes the parameter to be set and gets the session id */
	if ((ret = wlu_iovar_setbuf(wl, cmd->name, &flag, sizeof(uint32), buf,
					WLC_IOCTL_MAXLEN))) {
		printf("failed:%s %x\n",cmd->name,ret);
		return (ret);
	}
